...
async def menu_shop(callback: CallbackQuery):
    ...
    if not products:
        ...
        await callback.answer()
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{p['name']} | {p['price']}₽", callback_data=f"buy_{p['id']}")] for p in products] + [[InlineKeyboardButton(text="Назад", callback_data="menu_main", icon_custom_emoji_id=BUTTON_EMOJI["back"])]])
...
async def process_deposit_amount(message: Message, state: FSMContext):
    try:
        ...
        pending_payments[message.from_user.id] = {"amount": amount, "payment_id": payment_id, "status": "pending"}
        payment_url = await create_platega_payment(amount, payment_id, message.from_user.id)
        if not payment_url:
            await message.answer(f"{tg_emoji(STICKERS['keys_count'], '❌')} <b>Платежная система временно недоступна</b>\n\nСвяжитесь с администратором для пополнения баланса.\n\n👤 Админ: @nikita1055", parse_mode="HTML", reply_markup=get_profile_keyboard())
            return
        await message.answer(f"{tg_emoji(STICKERS['payment_method'], '💳')} <b>Оплата</b>\n\nСумма: <code>{amount} ₽</code>\n\n🔗 <a href='{payment_url}'>Нажмите для оплаты</a>\n\n🆔 ID платежа: <code>{payment_id}</code>", parse_mode="HTML", disable_web_page_preview=True, reply_markup=get_profile_keyboard())
    except ValueError:
...
async def handle_buy(callback: CallbackQuery):
    ...
    await callback.message.answer(f"{tg_emoji(STICKERS['product_selected'], '✅')} <b>Выбран товар • {product['name']}</b>\n\n{tg_emoji(STICKERS['keys_count'], '🔑')} <b>Ключей в наличии:</b> {keys_left}\n{tg_emoji(STICKERS['price_icon'], '💰')} <b>Цена:</b> {product['price']} ₽\n\n{tg_emoji(STICKERS['product_selected'], '🔑')} <b>Ваш ключ:</b> <code>{key_row['key_value']}</code>\n\n🔗 <b>Ссылка на VIP канал:</b>\n<a href='{vip_link}'>Нажмите для вступления</a>", parse_mode="HTML", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="В меню", callback_data="menu_main", icon_custom_emoji_id=BUTTON_EMOJI["home"])]]))
