...
def get_main_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['shop'], '🛍️')} Магазин", callback_data="menu_shop"), InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['profile'], '👤')} Профиль", callback_data="menu_profile")],
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['info'], 'ℹ️')} Информация", callback_data="menu_info")]
    ])

def get_profile_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['balance'], '💰')} Пополнить баланс", callback_data="profile_deposit"), InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['history'], '📋')} История заказов", callback_data="profile_history")],
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['promocode'], '🎫')} Активировать промокод", callback_data="profile_activate_promocode"), InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['referral'], '👥')} Реферальная система", callback_data="profile_referral")],
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['home'], '🏠')} Главное меню", callback_data="menu_main")]
    ])

def get_admin_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['add_product'], '➕')} Добавить товар", callback_data="admin_add_product"), InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['add_keys'], '🔑')} Добавить ключи", callback_data="admin_add_keys")],
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['add_balance'], '💰')} Выдать баланс", callback_data="admin_add_balance"), InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['broadcast'], '📢')} Сделать рассылку", callback_data="admin_broadcast")],
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['promocode'], '🎫')} Создать промокод", callback_data="admin_create_promocode"), InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['promocode'], '📋')} Список промокодов", callback_data="admin_list_promocodes")],
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['referral'], '🎁')} Настройка рефералов", callback_data="admin_ref_config")],
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['delete_product'], '📦')} Управление товарами", callback_data="admin_manage_products"), InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['delete_key'], '🔑')} Управление ключами", callback_data="admin_manage_keys")],
        [InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['stats'], '📊')} Статистика", callback_data="admin_stats"), InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['home'], '🏠')} Главное меню", callback_data="menu_main")]
    ])
...
async def menu_shop(callback: CallbackQuery):
    ...
    if not products:
        await callback.message.edit_text(f"{tg_emoji(STICKERS['keys_count'], '📭')} <b>Товаров пока нет</b>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['back'], '◀️')} Назад", callback_data="menu_main")]]))
        await callback.answer()
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['shop'], '🛍️')} {p['name']} | {p['price']}₽", callback_data=f"buy_{p['id']}")] for p in products] + [[InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['back'], '◀️')} Назад", callback_data="menu_main")]])
...
async def profile_referral(callback: CallbackQuery):
    ...
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['back'], '◀️')} Назад", callback_data="menu_profile")]]))
    await callback.answer()
...
async def profile_history(callback: CallbackQuery):
    ...
    if not purchases:
        await callback.message.edit_text(f"{tg_emoji(STICKERS['keys_count'], '📋')} <b>История заказов</b>\n\nУ вас пока нет покупок.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['back'], '◀️')} Назад", callback_data="menu_profile")]]))
        await callback.answer()
        return
    ...
    await callback.message.edit_text(history_text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['back'], '◀️')} Назад", callback_data="menu_profile")]]))
    await callback.answer()
...
async def profile_deposit(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"{tg_emoji(STICKERS['enter_amount'], '💰')} <b>Укажите сумму пополнения баланса</b>\n\nВведите сумму от 10 до 50000 ₽\n\nПример: <code>500</code>\n\nОтправьте число в этот чат", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['back'], '❌')} Отмена", callback_data="menu_profile")]]))
    await callback.answer()
...
async def profile_activate_promocode(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"{tg_emoji(BUTTON_EMOJI['promocode'], '🎫')} <b>Активация промокода</b>\n\nВведите промокод:\n\nПример: <code>SUMMER2024</code>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['back'], '❌')} Отмена", callback_data="menu_profile")]]))
    await callback.answer()
...
async def handle_buy(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{tg_emoji(BUTTON_EMOJI['home'], '🏠')} В меню", callback_data="menu_main")]])
    )
...
# Все admin обработчики (admin_add_product, admin_add_keys, admin_manage_products, admin_manage_keys, admin_add_balance, admin_broadcast, admin_create_promocode, admin_list_promocodes, admin_ref_config, admin_stats, admin_back) пишутся аналогично старой версии
...
async def admin_add_product(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📝 Введите название товара:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Отмена", callback_data="admin_back")]]))
    await callback.answer()
...
async def admin_add_keys(callback: CallbackQuery, state: FSMContext):
    ...
    if not products:
        ...
        await callback.answer()
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{p['name']} (ID: {p['id']})", callback_data=f"addkeys_{p['id']}")] for p in products] + [[InlineKeyboardButton(text="◀️ Назад", callback_data="admin_back")]])
...
async def select_for_keys(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🔑 Введите ключи (по одному на строку):", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Отмена", callback_data="admin_back")]]))
    await callback.answer()
...
async def admin_manage_products(callback: CallbackQuery):
    ...
    products = await get_all_products()
    if not products:
        await callback.message.edit_text("📭 <b>Список товаров пуст</b>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data="admin_back")]]))
        await callback.answer()
        return
...
async def admin_manage_keys(callback: CallbackQuery):
    ...
    products = await get_all_products()
    if not products:
        await callback.message.edit_text("📭 <b>Сначала добавьте товар</b>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data="admin_back")]]))
        await callback.answer()
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{p['name']} (ID: {p['id']})", callback_data=f"showkeys_{p['id']}")] for p in products] + [[InlineKeyboardButton(text="◀️ Назад", callback_data="admin_back")]])
...
async def show_keys(callback: CallbackQuery):
    ...
    if not keys:
        await callback.message.edit_text(f"🔑 <b>Ключи для товара {product['name']}</b>\n\nСписок пуст", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data="admin_manage_keys")]]))
        await callback.answer()
        return
    ...
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ Назад", callback_data="admin_manage_keys")]]))
    await callback.answer()
...
async def admin_add_balance(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("💰 <b>Выдача баланса пользователю</b>\n\nВведите ID пользователя Telegram:\n\nПример: <code>123456789</code>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Отмена", callback_data="admin_back")]]))
    await callback.answer()
...
async def process_add_balance_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        await bot.send_message(user_id, f"✅ <b>Баланс пополнен администратором!</b>\n\n💰 Сумма: <code>{amount} ₽</code>\n📊 Новый баланс: <code>{current_balance + amount} ₽</code>", parse_mode="HTML")
        await state.clear()
    except ValueError:
        await message.answer("❌ Введите число")
...
async def admin_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📢 <b>Рассылка сообщения</b>\n\nВведите текст сообщения для рассылки всем пользователям:\n\nПоддерживается HTML разметка", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Отмена", callback_data="admin_back")]]))
    await callback.answer()
...
async def process_broadcast(message: Message, state: FSMContext):
    ...
    for user in users:
        try:
            await bot.send_message(user["user_id"], f"📢 <b>РАССЫЛКА ОТ АДМИНИСТРАТОРА</b>\n\n{broadcast_text}", parse_mode="HTML")
            success_count += 1
        except:
            fail_count += 1
...
async def admin_create_promocode(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🎫 <b>Создание промокода</b>\n\nВведите название промокода (только латиница и цифры, без пробелов):\n\nПример: <code>SUMMER2024</code>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Отмена", callback_data="admin_back")]]))
    await callback.answer()
