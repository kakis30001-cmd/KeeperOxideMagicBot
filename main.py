...
async def start_cmd(message: Message):
    ...
    text = (
        ...
        f"{emoji(EMOJI['eye'], '👁️')} ESP (все видно)\n"
        f"{emoji(EMOJI['target'], '🎯')} Аимбот (НЕ НА ГОЛОВУ!)\n"
        f"{emoji(EMOJI['ghost'], '👻')} X-Ray (сквозь стены)\n"
        ...
        f"{emoji(EMOJI['heart'], '❤️')} <i>Приятных покупок!</i>"
    )
